...
def generate_user_content(item):
    question_translations = '\n'.join([f"Translation {idx + 1}: {item[f'question_vi_{source[idx]}']}" for idx in range(len(source))])
    answer_translations = '\n'.join([f"Translation {idx + 1}: {item[f'answer_vi_{source[idx]}']}" for idx in range(len(source))])

    # Handling explanations and their translations
    explanation_translations = []
    for exp_idx, exp in enumerate(item['explanation']):
        exp_translations = '\n'.join([f"Translation {trans_idx + 1}: {item[f'explanation_vi_{source[trans_idx]}'][exp_idx]}" 
                                      for trans_idx in range(len(source))])
        explanation_translations.append(f"Explanation {exp_idx + 1}: {exp}\n{exp_translations}")
...
def prepare_batch_input_files(input_file, dataset_name, batch_size=1500):
    ...
    for batch_idx, batch_keys in enumerate(batches):
        ...
        with open(os.path.join(batch_dir, batch_output_file), 'w', encoding='utf-8') as out_f:
...
def download_batch_results(batch_id):
    try:
        ...
        with open(os.path.join(batch_dir, output_file_path), 'w', encoding='utf-8') as output_file:
...
def parse_batch_results(result_file, original_data):
    ...
    for result in results:
        ...
        if key in original_data:
            ...
            parsed_result = {
                "id_image": key,
                "question": [item[f"question_vi_{source[i]}"] for i in range(len(source))],
                "question_scores": scores['question_scores'],
                "answer": [item[f"answer_vi_{source[i]}"] for i in range(len(source))],
                "answer_scores": scores['answer_scores'],
                "explanation": [[item[f"explanation_vi_{source[i]}"][j] for i in range(len(source))] for j in range(len(item['explanation']))],
...
def process_dataset(dataset_name):
    ...
    for batch_file in os.listdir(batch_dir):
        if batch_file.startswith(f'vqaX_{dataset_name}_batch') and batch_file.endswith('.jsonl'):
    ...
    final_output_file = os.path.join(evaluation_dir, f'{dataset_name}_gpt_evaluation.json')

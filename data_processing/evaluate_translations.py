...
def create_evaluation_prompt(question: str, answer: str, explanation: str, eval_type: str, translations: list[str]) -> str:
    prompt = f"""
...
def evaluate_translations(model_name, model, question: str, answer: str, explanation: str, eval_type: str, translations: list[str]) -> list[int]:
    prompt = create_evaluation_prompt(question, answer, explanation, eval_type, translations)
    try:
        with torch.no_grad():
...
def process_dataset(model_name, model, data, output_file):
    ...
    for sample_id, sample in tqdm(data.items(), desc=f"Processing {model_name}"):
        ...
        explanation_translations = [[sample[f"explanation_vi_{source}"][i] for source in sources] for i in range(len(sample["explanation"]))]

        question_scores = evaluate_translations(model_name, model, sample["question"], common_answer, sample["explanation"][0], "question", question_translations)
        answer_scores = evaluate_translations(model_name, model, sample["question"], common_answer, sample["explanation"][0], "answer", answer_translations)
        explanation_scores = [
            evaluate_translations(model_name, model, sample["question"], common_answer, sample["explanation"][i], "explanation", exp_translations)
...
def main(model_name):
    ...
    for dataset_name, file_path in dataset_files.items():
        ...
        output_file = os.path.join(output_dir, f'{dataset_name}_{model_name}_evaluation.json')
        process_dataset(model_name, model, data, output_file)
        print(f"Finished processing {dataset_name} dataset. Results saved to {output_file}")
